...
(async function () {
    ...
    // // 5 Posible values depending on what user selected: "disadvantage", "parity", "temporary", "unused" or "sustained"
    ...
    // CONFIGURATIONS OF THE TYPE OF GRAPHIC
    const config = {
        type: 'scatter',
        data: {
            datasets: [
                ...
                {
                    label: 'vrio',
                    // data: data.map(row => ({ x: row.precio, y: row.valor })) //Data not needed for this chart. Only annotations
                },
            ]
        },
        options,
    ...
    // This instanciates the chart with config and element
    new Chart(
        document.getElementById('vrio'),
        config
    );
})();